...
def  RanSamplerfunc(X,y):
	...
	rus = RandomUnderSampler(sampling_strategy={0:375},random_state=10) 
	#This is  modified depending on the number of samples
	# we are asking for 95 data points out of 375 in the first class. ## (y_train == 0).sum(), (y_train == 1).sum()

	nm = NearMiss(sampling_strategy={0: 375}) ####  This is modified depending on the number of samples.
...
def  OverSamplerfunc(X,y,rf):
	X_train, X_test, y_train, y_test = train_test_split(X, y, random_state=10, stratify=y)
	print((y_train == 0).sum(), (y_train == 1).sum())
	ros= RandomOverSampler(random_state=10,sampling_strategy={1:920})
	#up-sampling--- the minority class to have 920  instead
...
def     ANN_func(X,y):  
	...
	classifier.add(Dense(activation = "elu", input_dim = 7, units = 6, kernel_initializer = "uniform"))
